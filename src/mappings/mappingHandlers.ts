...
export async function handleNFTEvent(event: CosmosEvent): Promise<void> {
  ...
  if(data.action=="mint") {
    ...
    for await (const {key, value} of attr) {
      if(Number(key)>0) {
        let _new_ticketing_id = data._contract_address + key;
        const ticketingEntity = NftEntity.create({
          id: _new_ticketing_id,
          minter: data.minter,
          event_id: collection_data.event_id,
          owner: data.minter,
          event_infoId: collection_data.id,
          ticket_type: value
        });
        await ticketingEntity.save()
      }
    }
    return;
  }
